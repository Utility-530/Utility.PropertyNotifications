...
namespace UtilityStruct.FileSystem
{
    public ref struct Extension
    {
        ...
        {
            ExtensionContainsInvalidCharacter
        }

        public Extension(string name) : this(name.AsSpan())
{
            if (Validate(name) is { Failed: { Count: { } failures } failed } valids && failures > 0)
                throw new MultiValidationException<string>(valids);
        }

        public Extension(ReadOnlySpan<char> name)
        {
            Value = name;
        }

        public Extension(General name)
        {
            Value = name.ToString();
        }

        public Extension(MicrosoftOffice name)
        {
            Value = name.ToString();
        }
        ...
        public static ReadOnlySpan<char> MakeValid(string name)
        {
        ...
        public General ParseGeneral()
        {
            return (General)Enum.Parse(typeof(General), Value.ToString());
        }

        public MicrosoftOffice ParseMicrosoftOffice()
        {
            return (MicrosoftOffice)Enum.Parse(typeof(MicrosoftOffice), Value.ToString());
        }


        /// <summary>
        ///     Implicitly converts the specified <paramref name="extension"/> to its underlying path string.
        /// </summary>
        /// <param name="extension">The path to be converted.</param>
        /// <returns>
        ///     The path's underlying path string or <see langword="null"/> if <paramref name="extension"/>
        ///     is <see langword="null"/>.
        /// </returns>
        [return: NotNullIfNotNull("extension")]
        public static implicit operator Extension(string extension) => new Extension(extension);
    }
