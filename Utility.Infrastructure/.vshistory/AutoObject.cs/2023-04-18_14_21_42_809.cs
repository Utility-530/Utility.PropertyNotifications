...
namespace Utility.PropertyTrees.Infrastructure
{
    ...
    /// </summary>
    public abstract class AutoObject : IDataErrorInfo, INotifyPropertyChanged, IKey, IObserver, IGuid, IObserver<IPropertyChange>
    {
        ...
        public bool SetProperty(object value, Type type, [CallerMemberName] string name = null)
        {
            disposable ??= PropertyStore.Subscribe(this);
            var key = new Key(guid, name, type);
            var order = new Order { Key = key, OrderType = OrderType.Set, Value=value };
            //order.Subscribe(this);
            store[key] = value;
