...
{
    public ObservableDictionary(SynchronizationContext scheduler) : this()
    {
    ...
    public virtual void AddRange(IEnumerable<KeyValuePair<TKey, TValue>> keyValuePairs) =>
        AddRange(keyValuePairs.ToArray());
    ...
    public virtual IReadOnlyList<KeyValuePair<TKey, TValue>> GetRange(IEnumerable<TKey> keys) =>
        keys.Select(key => new KeyValuePair<TKey, TValue>(key, gd[key])).ToArray();
    ...
    protected virtual void OnCollectionChanged(NotifyCollectionChangedEventArgs e)
    {
        ...
        if (scheduler != null)
            scheduler.Post((o) => { CollectionChanged?.Invoke(this, e); }, null);
        else
    ...
    public virtual IReadOnlyList<TKey> ReplaceRange(IEnumerable<TKey> removeKeys, IEnumerable<KeyValuePair<TKey, TValue>> newKeyValuePairs)
    {
        ...
        var removingKeys = new HashSet<TKey>(removeKeys);
    ...
    public virtual TValue this[TKey key]
    {
        ...
        set
        {
            var oldValue = gd.ContainsKey(key) ? gd[key] : default;
            if (gd.ContainsKey(key))
            {
                if (ValueComparer.Equals(gd[key], value) == false)
                {
                    gd[key] = value;
    ...
    EventArgs
{
    ...
    private void InitializeRemove(NotifyDictionaryChangedAction action, IEnumerable<KeyValuePair<TKey, TValue>>? oldItems)
    {
        ...
        if (oldItems is IEnumerable<KeyValuePair<TKey, TValue>> actualOldItems)
            OldItems = actualOldItems.ToArray();
    }
    ...
    /// </summary>
    public IReadOnlyList<KeyValuePair<TKey, TValue>> NewItems { get; private set; } = Enumerable.Empty<KeyValuePair<TKey, TValue>>().ToArray();
    ...
    /// </summary>
    public IReadOnlyList<KeyValuePair<TKey, TValue>> OldItems { get; private set; } = Enumerable.Empty<KeyValuePair<TKey, TValue>>().ToArray();
}
