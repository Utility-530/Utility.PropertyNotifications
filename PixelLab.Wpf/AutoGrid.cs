...
namespace PixelLab.Wpf
{
    ...
    public class AutoGrid : Grid
    {
        ...
        /// Handle the columns changed event
        /// </summary>
        public static void ColumnsCountChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            if ((int?)e.NewValue == default)
                return;
        ...
        /// Handle the columns changed event
        /// </summary>
        public static void RowsCountChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            if ((int?)e.NewValue == default)
                return;
                ...
                !isVertical && _rowOrColumnCount != RowDefinitions.Count))
            {
                ...
                foreach (UIElement child in Children)
                {
                    if (IsAutoIndexing && GetAutoIndex(child) == true)
                    {
                        ...
                        {
                            if (DependencyPropertyHelperEx.HasProperty(child, Grid.RowProperty) == false)
                            {
                                rowIndex = cellPosition / ColumnDefinitions.Count;
                                SetRow(child, rowIndex);
                            }
                            else
                                rowIndex = GetRow(child);

                            if (DependencyPropertyHelperEx.HasProperty(child, Grid.ColumnProperty) == false)
                            {
                                columnIndex = cellPosition % ColumnDefinitions.Count;
                            ...
                            {
                                if (RowDefinitions.Count <= rowIndex)
                                {
                            ...
                            {
                                if(ColumnDefinitions.Count<= columnIndex)
                                {
                        ...
                        else
                        {
                            if (DependencyPropertyHelperEx.HasProperty(child, Grid.RowProperty) == false)
                            {
                                rowIndex = cellPosition % RowDefinitions.Count;
                                SetRow(child, rowIndex);
                            }
                            else
                                rowIndex = GetRow(child);
                            ...
                            if (DependencyPropertyHelperEx.HasProperty(child, Grid.ColumnProperty) == false)
                            {
                                columnIndex = cellPosition / RowDefinitions.Count;
                                SetColumn(child, columnIndex);
                            }
                            else
                                columnIndex = GetColumn(child);
      ...
      DependencyProperty.RegisterAttached("ColumnsCount", typeof(int?), typeof(AutoGrid),
          new FrameworkPropertyMetadata(default,
              FrameworkPropertyMetadataOptions.AffectsMeasure | FrameworkPropertyMetadataOptions.AffectsArrange,
              new PropertyChangedCallback(ColumnsCountChanged)));

        public static readonly DependencyProperty RowsCountProperty =
      DependencyProperty.RegisterAttached("RowsCount", typeof(int?), typeof(AutoGrid),
          new FrameworkPropertyMetadata(default,
              FrameworkPropertyMetadataOptions.AffectsMeasure | FrameworkPropertyMetadataOptions.AffectsArrange,
              new PropertyChangedCallback(RowsCountChanged)));
    ...
    class DependencyPropertyHelperEx
    {
        // Check if an object has the attached property set
        public static bool HasProperty(DependencyObject obj, DependencyProperty dependencyProperty)
        {
            object value = obj.GetValue(dependencyProperty);
