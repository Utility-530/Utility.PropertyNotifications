...
namespace NetPrints.Graph
{
    ...
    public class ConstructorNode : ExecNode
    {
        ...
        /// </summary>
        public IBaseType ClassType
        {
            get => OutputDataPins[0].PinType;
        }
        ...
        {
            get => ArgumentPins.Select(p => p.PinType).ToList();
        }
        ...
        public ConstructorNode(INodeGraph graph, ConstructorSpecifier specifier)
            : base(graph)
        {
        ...
        private void UpdateTypes()
        {
            ...
            for (int i = 0; i < ConstructorSpecifier.Arguments.Count; i++)
            {
                ...
                if (InputDataPins[i].PinType != constructedType)
                {
                    InputDataPins[i].PinType = constructedType;
                }
            }

            // Construct data output
            {
                IBaseType constructedType = GenericsHelper.ConstructWithTypePins(ConstructorSpecifier.DeclaringType, InputTypePins);
                OutputDataPins[0].PinType = constructedType;
            }
        }
