    ...
    public class CallMethodNode : ExecNode
    {
        ...
        public string BoundMethodName
        {
            get
            {
                ...
                {
                    boundName += $"<{string.Join(",", InputTypePins.Select(p => p.InferredType?.FullCodeName ?? p.Name))}>";
                }
        ...
        {
            get => InputDataPins.Select(p => p.PinType).ToList();
        }

        /// <summary>
        /// List of named type specifiers the method takes.
        /// </summary>
        public IReadOnlyList<Named<IBaseType>> Arguments
        {
            get => InputDataPins.Select(p => new Named<IBaseType>(p.Name, p.PinType)).ToList();
        }

        /// <summary>
        /// List of type specifiers the method returns.
        /// </summary>
        public IReadOnlyList<IBaseType> ReturnTypes
        {
            get => OutputDataPins.Select(p => p.PinType).ToList();
        }

        /// <summary>
        /// Target ("this") to call the method on.
        /// </summary>
        public INodeInputDataPin TargetPin
        {
        ...
        /// </summary>
        public INodeOutputDataPin ExceptionPin
        {
        ...
        /// </summary>
        public INodeOutputExecPin CatchPin
        {
        ...
        /// </summary>
        public IList<INodeInputDataPin> ArgumentPins
        {
            get
            {
                if (IsStatic)
        ...
        /// </summary>
        public IList<INodeOutputDataPin> ReturnValuePins
        {
            get => (OutputDataPins.Where(p => p.Name != ExceptionPinName)).ToList();
        }

        public CallMethodNode(INodeGraph graph, MethodSpecifier methodSpecifier,
        ...
        private void UpdateTypes()
        {
            ...
            {
                IBaseType type = MethodSpecifier.Parameters[i].Value;

                // Construct type with generic arguments replaced by our input type pins
                IBaseType constructedType = GenericsHelper.ConstructWithTypePins(type, InputTypePins);

                if (ArgumentPins[i].PinType != constructedType)
                {
                    ArgumentPins[i].PinType = constructedType;
                }
            }

            for (int i = 0; i < MethodSpecifier.ReturnTypes.Count; i++)
            {
                IBaseType type = MethodSpecifier.ReturnTypes[i];

                // Construct type with generic arguments replaced by our input type pins
                IBaseType constructedType = GenericsHelper.ConstructWithTypePins(type, InputTypePins);

                // +1 because the first pin is the exception pin
                if (ReturnValuePins[i].PinType != constructedType)
                {
                    ReturnValuePins[i].PinType = constructedType;
                }
            }
        }
