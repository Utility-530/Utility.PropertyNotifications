...
namespace NetPrints.Graph
{
    ...
    public class RerouteNode : Node
    {
        ...
        private RerouteNode(INodeGraph graph)
            : base(graph)
        {
        }

        public static RerouteNode MakeExecution(INodeGraph graph, int numExecs)
        {
        ...
        public static RerouteNode MakeData(INodeGraph graph, IEnumerable<Tuple<IBaseType, IBaseType>> dataTypes)
        {
        ...
        public static RerouteNode MakeType(INodeGraph graph, int numTypes)
        {
            var node = new RerouteNode(graph);
            ...
            for (int i = 0; i < numTypes; i++)
            {
                ...
                node.AddOutputTypePin($"Type{i}", new ObservableValue<IBaseType>(null));
            }
        ...
        private void UpdateOutputType()
        {
            ...
            {
                OutputTypePins[0].InferredType = InputTypePins[0].InferredType;
            }
        }
