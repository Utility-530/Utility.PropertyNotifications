namespace NetPrints.Translator
{
    ...
    /// </summary>
    public class ClassTranslator:IClassTranslator
    {
        ...
        public string TranslateClass(IClassGraph c)
        {
            StringBuilder content = new StringBuilder();

            foreach (IVariable v in c.Variables)
            {
                content.AppendLine(TranslateVariable(v));
            }

            foreach (IConstructorGraph constructor in c.Constructors)
            {
                content.AppendLine(TranslateConstructor(constructor));
            }

            foreach (IMethodGraph m in c.Methods)
            {
        ...
        public string TranslateVariable(IVariable variable)
        {
            StringBuilder modifiers = new StringBuilder();
        ...
        public string TranslateMethod(IMethodGraph m)
        {
            return methodTranslator.Translate(m, true);
        }

        public string TranslateConstructor(IConstructorGraph m)
        {
            return methodTranslator.Translate(m, true);
        }
    }
