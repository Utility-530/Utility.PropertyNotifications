...
namespace NetPrints.Translator
{
    public static class TranslatorUtil
    {
        ...
        private static void AddAllNodes(INode node, ref HashSet<INode> nodes)
        {
            nodes.Add(node);

            foreach (INodeOutputExecPin pin in node.OutputExecPins)
            {
            ...
            foreach (INodeInputDataPin pin in node.InputDataPins)
            {
            ...
            foreach (INodeInputTypePin pin in node.InputTypePins)
            {
                if (pin.IncomingPin != null && !nodes.Contains(pin.IncomingPin.Node))
                {
                    AddAllNodes(pin.IncomingPin.Node, ref nodes);
        ...
        public static IEnumerable<INode> GetAllNodesInExecGraph(IExecutionGraph graph)
        {
        ...
        private static void AddExecNodes(INode node, ref HashSet<INode> nodes)
        {
            nodes.Add(node);
        ...
        public static IEnumerable<INode> GetExecNodesInExecGraph(IExecutionGraph graph)
        {
            HashSet<INode> nodes = new HashSet<INode>();
        ...
        private static void AddDependentPureNodes(INode node, ref HashSet<INode> nodes)
        {
        ...
        public static IEnumerable<INode> GetDependentPureNodes(INode node)
        {
            HashSet<INode> nodes = new HashSet<INode>();
        ...
        public static IEnumerable<INode> GetSortedPureNodes(INode node)
        {
            var dependentNodes = GetDependentPureNodes(node);
            List<INode> remainingNodes = new List<INode>(dependentNodes);
            ...
            do
            {
                ...
                foreach (INode evalNode in remainingNodes)
                {
                ...
                foreach (INode newNode in newNodes)
                {
