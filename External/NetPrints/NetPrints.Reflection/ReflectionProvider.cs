...
namespace NetPrintsEditor.Reflection
{
    ...
    public class ReflectionProvider : IReflectionProvider
    {
        ...
        private readonly Lazy<List<IMethodSymbol>> extensionMethods;
        ...
        public ReflectionProvider(IEnumerable<string> assemblyPaths, IEnumerable<string> sourcePaths, IEnumerable<string> sources)
        {
            reflectionHelpers = new(() =>
            {
                ...
                var syntaxTrees = sources.Select(source => X.ParseSyntaxTree(source));
                ...
                (EmitResult compilationResults, Stream stream) = X.CompileInMemory(compilation);
                ...
                return new ReflectionHelpers(compilation, documentationUtil);
            });


            extensionMethods = new(() =>
            {
                return new List<IMethodSymbol>(reflectionHelpers.Value.GetValidTypes().SelectMany(t => t.GetMethods().Where(m => m.IsExtensionMethod)));
            });
        }
        ...
        public IEnumerable<IMethodSpecifier> GetMethods(IReflectionProviderMethodQuery? query)
        {
            ...
            if (query.Type is not null)
            {
                ...
                var extensions = extensionMethods.Value.Where(m => type.IsSubclassOf(m.Parameters[0].Type)).ToList();
...
public static ReflectionProvider From(IProject project) => X.From(project);
    }
