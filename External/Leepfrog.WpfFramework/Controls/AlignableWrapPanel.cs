...
namespace Leepfrog.WpfFramework.Controls
{
    public class AlignableWrapPanel : Panel
    {
        ...
        {
            double x = 0;
            double extraSpace = (boundsWidth - lineSize.Width);
            ...
            for (int i = start; i < end; i++)
            {
                ...
                var thisExtraSpace = extraSpace / (end - i);
                var thisWidth = child.DesiredSize.Width + thisExtraSpace;
                child.Arrange(new Rect(x, y, thisWidth, lineSize.Height));
                x += thisWidth;
                extraSpace -= thisExtraSpace;
            }
        }
    }
}