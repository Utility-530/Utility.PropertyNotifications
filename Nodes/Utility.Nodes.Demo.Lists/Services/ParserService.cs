...
{
    internal record InNodeParam() : Param<ParserService>(nameof(ParserService.AddElementByPositionAsync), "node");
    internal record OutStringParam() : Param<ParserService>(nameof(ParserService.AddElementByPositionAsync));
...
<body/>
        ...
        static IDocument create(Dictionary<IReadOnlyTree, IElement> dictionary, IReadOnlyTree node, IReadOnlyTree n, IDocument document)
        {
            ...
            else if ((n as IGetParent<IReadOnlyTree>).Parent is ICollectionDescriptor _prsdescriptor && n is IValueDescriptor descriptor1)
            {
                newElement = document.CreateElement<IHtmlTableDataCellElement>();
                newElement.TextContent = (descriptor1 as IGetValue).Value?.ToString();
            }
            else if (n is IValueDescriptor valueDescriptor)
            {
                newElement = document.CreateElement<IHtmlDivElement>();
            }
            else
            {
            ...
            if (n is IDescriptor descriptor)
            {
                ...
                {
                    var innerElement = create(descriptor1.Value());
            ...
            IElement create(object value)
            {
                switch (value)
                {
                    case null:
                        {
                            var _p = document.CreateElement<IHtmlParagraphElement>();
                            _p.SetStyle("padding:10px 14px;\r\n                border:1px solid #d7c8f0;\r\n                border-radius:6px;\r\n                margin-bottom:16px;\r\n                background:#ffffff;\r\n                font-family:Segoe UI, sans-serif;");
                    ...
                    case string str:
                        {
                            ...
                            p.SetAttribute("background", "#f8f1ff");
                            p.SetAttribute("padding", "10px 14px");
                            p.SetAttribute("border-radius", "6px");
                            p.SetAttribute("border", "1px solid #d7c8f0");
                            p.SetAttribute("font-family", "Segoe UI, sans-serif");
                            p.SetAttribute("font-size", "14px");
                            p.SetAttribute("line-height", "1.4");
                            p.TextContent = str;
                            return p;
                        }
                    case bool b:
