...
namespace Utility.Nodes.Demo.Styles
{
    public class UpdateControlBehavior : Behavior<Control>
    {
        ...
        {
            if (sender is UpdateControlBehavior behavior /*&& e.NewValue.GetType() != behavior.Type*/)
                behavior.Update();
        }

        public UpdateControlBehavior() : base() { }
        ...
        void Update()
        {
            ...
            {
                AssociatedObject.Dispatcher.Invoke(()=> { }, DispatcherPriority.Render);
            }
        }
    }
}
