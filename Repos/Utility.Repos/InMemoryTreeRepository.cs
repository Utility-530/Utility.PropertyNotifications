...
namespace Utility.Repos
{
    public class InMemoryTreeRepository : ITreeRepository
    {
        ...
        public IObservable<Changes.Set<Key>> Find(Guid? parentGuid = default, string? name = null, Guid? guid = null, System.Type? type = null, int? index = null)
        {
            ...
            return Observable.Create<Changes.Set<Key>>(observer =>
            {
                ...
                if (tables.Count == 0)
                {
                    ...
                    {
                        observer.OnNext(new Changes.Set<Key>());
                    ...
                    .Subscribe(a =>
                    {
                        if (a is Guid guid)
                        {
                            observer.OnNext(new(Changes.Change.Add(new Key(guid, parentGuid.Value, type, name, index, default))));
                        }
                        else
                ...
                else if (tables.Count == 1)
                {
                    ...
                    observer.OnNext(new(Changes.Change.Add(new Key(table.Guid, parentGuid.Value, table.TypeId.HasValue ? ToType(table.TypeId.Value) : null, table.Name, index, table.Removed))));
                    observer.OnCompleted();
                }
                else if (name == null)
                {
                    List<Changes.Change<Key>> changes = new();
